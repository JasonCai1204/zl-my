...
<div class="container" id="container_order">
    <form action="/orders/create" method="post">
        ...
        <div class="weui-cells">
            ...
            <a href="/hospital/select?{{ isset($hospital) ? 'hospital_id=' . $hospital->id . '&' : ''}}{{ isset($doctor) ? 'doctor_id=' . $doctor->id . '&' : '' }}{{ isset($instance) ? 'instance_id=' . $instance->id . '&' : '' }}{{isset($city) ? 'city_id=' . $city->id : '' }}"
            ...
            <a href="/doctor/select?{{ isset($hospital) ? 'hospital_id=' . $hospital->id . '&' : ''}}{{ isset($doctor) ? 'doctor_id=' . $doctor->id . '&' : '' }}{{ isset($instance) ? 'instance_id=' . $instance->id : '' }}{{isset($city) ? 'city_id=' . $city->id : '' }}"
            ...
            <a href="/instance/select?{{ isset($hospital) ? 'hospital_id=' . $hospital->id . '&' : ''}}{{ isset($doctor) ? 'doctor_id=' . $doctor->id . '&' : '' }}{{ isset($instance) ? 'instance_id=' . $instance->id : '' }}{{isset($city) ? 'city_id=' . $city->id : '' }}"
...
<div class="js_dialog" id="FailConfirm" style="display: none;">
    <div class="weui-mask"></div>
    <div class="weui-dialog">
        <div id="FailConfirm_bd" class="weui-dialog__bd">
            未上传成功的图片将会丢失，您确定要离开页面吗？
        </div>
        <div class="weui-dialog__ft">
            <a href="javascript:;" id="FailConfirmTrue" class="weui-dialog__btn weui-dialog__btn_primary">离开</a >
            <a href="javascript:;" id="FailConfirmFalse" class="weui-dialog__btn weui-dialog__btn_primary">不离开</a >
        </div>
    </div>
</div>

<div class="js_dialog" id="LoadingConfirm" style="display: none;">
    <div class="weui-mask"></div>
    <div class="weui-dialog">
        ...
        <div class="weui-dialog__ft">
            <a href="javascript:;" id="LoadingConfirmTrue" class="weui-dialog__btn weui-dialog__btn_primary">好</a>
        </div>
    </div>
</div>
...
@section('script')
<script src="/js/user/exif.js"></script>
<script type="text/javascript">
    ...
    function photoConfirm(dom) {
        if ($(".weui-uploader__file_status").length >= 1) {
            event.preventDefault();
            if($(".weui-icon-warn").length >=1){
                $("#FailConfirm_bd").text('未上传成功的图片将会丢失，您确定要离开页面吗？');
                $("#FailConfirmTrue").text('离开');
                $("#FailConfirmFalse").text('不离开');
                $("#FailConfirm").fadeIn();
                $("#FailConfirmFalse").click(function () {
                    $("#FailConfirm").fadeOut();
                });
                $("#FailConfirmTrue").click(function () {
                    writeCookie();
                    $("#FailConfirm").fadeOut();
                    window.location.href =dom.attr('href');
                })
            }else if($(".weui-loading").length >=1){
                $("#LoadingConfirm").fadeIn();
                $("#LoadingConfirmTrue").click(function () {
                    $("#LoadingConfirm").fadeOut();
                })
            }
        }else{
            writeCookie();
    ...
    $('[type = "submit"]').on('click', function () {
        if ($(".weui-uploader__file_status").length >= 1) {
            event.preventDefault();
            if($(".weui-icon-warn").length >=1){
                $("#FailConfirm_bd").text('未上传成功的图片将会丢失，您确定要现在预约吗？');
                $("#FailConfirmTrue").text('预约');
                $("#FailConfirmFalse").text('暂不');
                $("#FailConfirm").fadeIn();
                $("#FailConfirmFalse").click(function () {
                    $("#FailConfirm").fadeOut();
                });
    ...
    function clickPhoto($obj) {
        ...
        if (thisfile.has($(".weui-icon-warn")).length >= 1) {
            ...
            $("#gallertImg").css({
                ...
                'transform': 'rotate(' + thisfile.attr('data-deg') + 'deg)'
            });
            $("#gallery").show().css({
                'background-size': '100%',
    ...
    if (document.cookie != "") {
        ...
        if (getCookie('uploadStr')) {
            ...
            for (var i = 0; i < uploadArr.length; i++) {
                ...
                if (uploadArr[i][1] == 1) {
                    deg = 0
                }else if (uploadArr[i][1] == 3) {
                    deg = 180;
                } else if (uploadArr[i][1] == 6) {
                    deg = 90;
                } else if (uploadArr[i][1] == 8) {
                    deg = 270;
                }
                if (uploadArr[i][2] != '') {
                    if (uploadArr[i][2] != undefined) {
                        ($("<li class='weui-uploader__file'></li>")
                                .css({
                                    'background-image': 'url("/storage/' + uploadArr[i][2] + '")',
                                ...
                                .html('<input name="photos[]" type="hidden" value="' + uploadArr[i][2] + '" />')
    ...
    $(".weui-uploader__input").change(function () {
        ...
        for (var i = 0; i < this.files.length; i++) {
            ...
            getPhotoOrientation(this.files[i], uploadArr[Arrlength + i], uploadShow);
    ...
    function getPhotoOrientation(img, arr, dom) {
        var orient;
        EXIF.getData(img, function () {
            orient = EXIF.getTag(this, 'Orientation');
            arr.push(orient);
            if (orient == 1) {
                dom.css("transform", "rotate(0deg)").attr('data-deg', '0');
            }else if (orient == 3) {
                dom.css("transform", "rotate(180deg)").attr('data-deg', '180');
            } else if (orient == 6) {
                dom.css("transform", "rotate(90deg)").attr('data-deg', '90');
            } else if (orient == 8) {
                dom.css("transform", "rotate(270deg)").attr('data-deg', '270');
            }
        });
        return orient;
    };
    ...
    function upload(formData, dom, arr) {
        $.ajax({
            ...
            false: function () {
                dom.attr({'data_flag': 'false', 'onclick': 'clickPhoto($(this))'})
                        .html("<div class='weui-uploader__file-content'><i class='weui-icon-warn' style='transform:rotate(-" +
                                dom.attr('data-deg') +
                                "deg)'></i></div>");
                arr.push('')
            }
            ...
            dom.attr({'data_flag': 'false', 'onclick': 'clickPhoto($(this))'})
                    .html(
                            "<div class='weui-uploader__file-content'><i class='weui-icon-warn' style='transform:rotate(-"
                            + dom.attr('data-deg')
                            + "deg)' ></i></div>");
            arr.push('')
        });
    ...
    $(".my_order_reuploader").on('click', function () {
        ...
        $.ajax({
            ...
            dataType: 'json',
            success: function (data) {
                thisfile.removeClass('weui-uploader__file_status').attr("onclick", "clickPhoto($(this))")
                        .html('<input name="photos[]" type="hidden" value="' + data.file.paths[0] + '" />');
                uploadArr[fileindex][2] = data.file.paths[0];
            },
            false: function () {
                thisfile.attr({'data_flag': 'false', 'onclick': 'clickPhoto($(this))'})
                        .html(
                                "<div class='weui-uploader__file-content'><i class='weui-icon-warn' style='transform:rotate(-"
                                + $(this).attr('data-deg')
                                + "deg)' ></i></div>");
                uploadArr[fileindex][2] = '';
            }
        });
    });
